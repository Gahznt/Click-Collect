...
session_start();
 ...
 $html .= "<table border = '1'>";
 $html .= '<tr>';
 $html .= '<td colspan="5" align="center"style="background-color:blue; color:white;"><b>Relatório CLICK</b></td>';
 $html .= '<td colspan="4" align="center"style="background-color:red; color:white;"><b>Infs de entrega em loja</b></td>';
 $html .= '</tr>';
...
if ($loja != "todos") {
	$query = "SELECT * FROM recebimento_loja RIGHT JOIN despacho ON (recebimento_loja.pedido = despacho.gaiolas) WHERE loja = '".$loja."' OR loja_entrega = '".$loja."' AND  DATE(datadespacho) between '".$dtinicio."' and '".$dtfim."';";
	$resultado_query = mysqli_query($conexao, $query);
} else {
	$query = "SELECT * FROM recebimento_loja RIGHT JOIN despacho ON (recebimento_loja.pedido = despacho.gaiolas) WHERE loja <> 'CT PB' AND loja <> 'CT RJ' AND loja <> 'CT SC' AND DATE(datadespacho) between '".$dtinicio."' and '".$dtfim."';";
	$resultado_query = mysqli_query($conexao, $query);
}
